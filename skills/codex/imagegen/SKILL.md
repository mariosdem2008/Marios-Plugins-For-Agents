---
name: "imagegen"
description: "Use when the user asks to generate or edit images (for example: generate image, edit/inpaint/mask, background removal or replacement, transparent background, product shots, concept art, covers, or batch variants), using a hosted imagegen tool when available or the bundled OpenAI Image API CLI as fallback."
---


# Image Generation Skill

Generates or edits images for the current project (e.g., website assets, game assets, UI mockups, product mockups, wireframes, logo design, photorealistic images, infographics). Prefer a hosted/callable `imagegen` or `image_gen` tool when the runtime exposes one. Use the bundled CLI for deterministic, reproducible local runs or when no hosted tool is available.
...
5. Generate the image:
   - If the runtime exposes a callable `imagegen` / `image_gen` tool, call it directly with the structured prompt. This route does not require the local `OPENAI_API_KEY` environment variable.
   - Otherwise, run the bundled CLI (`scripts/image_gen.py`) with sensible defaults (see references/cli.md).
...
## Environment
- Hosted/callable `imagegen` tool route: no local `OPENAI_API_KEY` is required.
- Bundled CLI route: `OPENAI_API_KEY` must be set for live API calls.

If the CLI route is the only available route and the key is missing, give the user these steps:
...
- Require `OPENAI_API_KEY` before any live CLI/API call, but do not require it before using a hosted/callable imagegen tool.
